...
<h6 class="text-center p-2 bg-red-800"><span class='text-md text-white mt-3'>
{{session('del')}}
</span></h6>
@endif
...
<td class="px-6 py-4 font-medium text-gray-800">{{$row->fullname}}</td>
<td class="px-6 py-4 text-gray-600">{{$row->email}}</td>
<td class="px-6 py-4 text-gray-600">{{$row->mobile}}</td>
...
<a href='{{asset("/admin-login/manage-contacts/".$row->id)}}'><button onclick="return confirm('Are you sure to delete Data ?')"
